...
library(MASS)
library(leaps)
modelsf <- regsubsets(x=xx,nbest=20,nvmax=40,y=fullmodel$model$DRVGR2011_RV.Graduation.rate...Bachelor.degree.within.5.years..total,method="forward",really.big=T)
modelsb <- regsubsets(x=xx,nbest=20,nvmax=40,y=fullmodel$model$DRVGR2011_RV.Graduation.rate...Bachelor.degree.within.5.years..total,method="backward",really.big=T)
modelss <- regsubsets(x=xx,nbest=20,nvmax=40,y=fullmodel$model$DRVGR2011_RV.Graduation.rate...Bachelor.degree.within.5.years..total,method="seqrep",really.big=T)


# get best model from both forward, backwards, and sequential deletions
whichmodelf <- summary(modelsf)$which[summary(modelsf)$rss == min(summary(modelsf)$rss),]
whichmodelb <- summary(modelsb)$which[summary(modelsb)$rss == min(summary(modelsb)$rss),]
whichmodels <- summary(modelss)$which[summary(modelss)$rss == min(summary(modelss)$rss),]
...
data.m <- data.tmp[,names(data.tmp)[names(data.tmp) %in% setdiff(c(varsin,facs,resp),names(missing)[missing > 300])]]
...
#do automated variable selection both forwards and backwards in an attempt to find a starting model, get a liberal number of variables
#exhaustive search takes a long long time with this many variables.
modelsf <- regsubsets(x=xx,nbest=20,nvmax=27,y=fullmodel$model$DRVGR2011_RV.Graduation.rate...Bachelor.degree.within.5.years..total,method="forward",really.big=T)
modelsb <- regsubsets(x=xx,nbest=20,nvmax=27,y=fullmodel$model$DRVGR2011_RV.Graduation.rate...Bachelor.degree.within.5.years..total,method="backward",really.big=T)
modelss <- regsubsets(x=xx,nbest=20,nvmax=27,y=fullmodel$model$DRVGR2011_RV.Graduation.rate...Bachelor.degree.within.5.years..total,method="seqrep",really.big=T)
