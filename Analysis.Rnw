...
gradrates <- setdiff(names(data)[union(grep(".Graduation.rate..",names(data),fixed=T),grep(".Grauduation.rate..",names(data),fixed=T))],c("DRVGR2011_RV.Graduation.rate...Bachelor.degree.within.5.years..total"))

row.names(data) <- paste0(as.character(data$institution.name),as.character(data$HD2011.FIPS.state.code))
data <- data[,setdiff(names(data),c(ids,gradrates))]
...
mis <- names(missing)[missing > 218]

#other selected drops
other <- c("SFA1011_RV.Average.amount.of.other.student.loan.aid.received.by.full.time.first.time.undergraduates",
...
#missing data assessment
missing <- apply(data.tmp,2,function(x) {return(sum(is.na(x)))})

#remove single factors
test <- apply(data.tmp2,2,unique)
test2 <- unlist(lapply(test,length))
singlefactors <- names(which(test2 == 1))
data.tmp2 <- data.tmp2[,setdiff(names(data.tmp2),singlefactors)]

#automatic variable selection:
fullmodel <- lm(DRVGR2011_RV.Graduation.rate...Bachelor.degree.within.5.years..total~.,data=data.tmp2)
...
# get bic for every subset regression
models <- regsubsets(x=x,
                     y=fullmodel$model$DRVGR2011_RV.Graduation.rate...Bachelor.degree.within.5.years..total,
                     method="exhaustive",
                     nbest=50, # get all combinations of predictors
                     really.big=T)
@
