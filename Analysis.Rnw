...
if(!file.exists("CleanData.csv")) {
  write.csv(data.tmp,file="CleanData.csv",row.names=F)
}
...
#do automated variable selection both forwards and backwards in an attempt to find a starting model, get a liberal number of variables
#exhaustive search takes a long long time with this many variables.
modelsf <- regsubsets(x=xx,nbest=20,nvmax=26,y=fullmodel$model$DRVGR2011_RV.Graduation.rate...Bachelor.degree.within.5.years..total,method="forward",really.big=T)
modelsb <- regsubsets(x=xx,nbest=20,nvmax=26,y=fullmodel$model$DRVGR2011_RV.Graduation.rate...Bachelor.degree.within.5.years..total,method="backward",really.big=T)
modelss <- regsubsets(x=xx,nbest=20,nvmax=26,y=fullmodel$model$DRVGR2011_RV.Graduation.rate...Bachelor.degree.within.5.years..total,method="seqrep",really.big=T)
...
if(!file.exists("MinimalData.csv")) {
  write.csv(data.m,file="MinimalData.csv",row.names=F)
}
@
