...
mis <- names(missing)[missing > 283]
data <- data[,setdiff(names(data),c(mis))]

#get rid of those schools that didn't respond to the latest questions
data.tmp = na.omit(data)

#tests
test <- apply(data.tmp,2,unique)
test2 <- unlist(lapply(test,length))
singlefactors <- names(which(test2 == 1))
data.tmp <- data.tmp[,setdiff(names(data.tmp),singlefactors)]
