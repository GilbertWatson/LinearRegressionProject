...
derived <- as.data.frame(derived)[!is.na(data$DRVIC2011_RV.Percent.admitted...total),]
@
\subsection{Initial Model Results:}

<<tidyup,echo=TRUE>>=
#remove responses from data and observations with no response
responsevars <- c("DRVIC2011_RV.Percent.admitted...men","DRVIC2011_RV.Percent.admitted...women")
data <- data[!is.na(data$DRVIC2011_RV.Percent.admitted...total),setdiff(names(data),responsevars)]
...
fullmodel <- lm(DRVIC2011_RV.Percent.admitted...total~.,data=data.tmp)
...
library(MASS)
library(leaps)
modelsf <- regsubsets(x=xx,nbest=20,nvmax=50,y=fullmodel$model$DRVIC2011_RV.Percent.admitted...total,method="forward",really.big=T)
modelsb <- regsubsets(x=xx,nbest=20,nvmax=50,y=fullmodel$model$DRVIC2011_RV.Percent.admitted...total,method="backward",really.big=T)
modelss <- regsubsets(x=xx,nbest=20,nvmax=50,y=fullmodel$model$DRVIC2011_RV.Percent.admitted...total,method="seqrep",really.big=T)
...
testmodel <- lm(fullmodel$model$DRVIC2011_RV.Percent.admitted...total~xx[,which(inatleastone[2:length(inatleastone)] == T)])
@
...
facs <- c("HD2011.Institution.size.category",
          ...
          "HD2011.Carnegie.Classification.2010..Basic")

# and of course the response variable
resp <- c("DRVIC2011_RV.Percent.admitted...total")
...
fullmodel <- lm(DRVIC2011_RV.Percent.admitted...total~.,data=data.m)
xx<-model.matrix(fullmodel)[,-1]

#do automated variable selection both forwards and backwards in an attempt to find a starting model, get a liberal number of variables
#exhaustive search takes a long long time with this many variables.
modelsf <- regsubsets(x=xx,nbest=20,nvmax=20,y=fullmodel$model$DRVIC2011_RV.Percent.admitted...total,method="forward",really.big=T)
modelsb <- regsubsets(x=xx,nbest=20,nvmax=20,y=fullmodel$model$DRVIC2011_RV.Percent.admitted...total,method="backward",really.big=T)
modelss <- regsubsets(x=xx,nbest=20,nvmax=20,y=fullmodel$model$DRVIC2011_RV.Percent.admitted...total,method="seqrep",really.big=T)
...
testdata <- as.data.frame(cbind(fullmodel$model$DRVIC2011_RV.Percent.admitted...total,xx[,which(inatleastone[2:length(inatleastone)] == T)]))
names(testdata)[1] <- "DRVIC2011_RV.Percent.admitted...total"
testmodel <- lm(DRVIC2011_RV.Percent.admitted...total~.,data=testdata)
xx <- model.matrix(testmodel)[,-1]

#branch and prune selection
models <- leaps(x=xx,y=testmodel$model$DRVIC2011_RV.Percent.admitted...total,method="adjr",nbest=3)

#estimate the best model
lastdata <- as.data.frame(cbind(testmodel$model$DRVIC2011_RV.Percent.admitted...total,xx[,which(models$which[(models$adjr2 == max(models$adjr2)),])]))
names(lastdata)[1] <- "DRVIC2011_RV.Percent.admitted...total"
lastmodel <- lm(DRVIC2011_RV.Percent.admitted...total~.,data=lastdata)
summary(lastmodel)
